...
狀態轉換是 Ethereum 的核心計算過程：給定當前世界狀態 $\sigma$ 和一筆交易 $T$，EVM 執行後產生新狀態 $\sigma'$。所有帳戶餘額、合約儲存、nonce 的變更都反映在 [[Merkle Patricia Trie]] 中，新的 state root 寫入 [[區塊 Header]]。這是交易生命週期的最後一步 -- 交易的效果在此刻實際生效。
