...
區塊打包完成後，怎麼確保它不會被推翻？這就是共識與最終性要解決的問題。Ethereum 使用兩個協議搭配運作：[[LMD GHOST]] 負責即時決定「目前哪條鏈最長」（fork choice），[[Casper FFG]] 負責在較長時間尺度上蓋棺定論——一旦 finalize，交易就不可逆轉。

一個 epoch 是 32 個 slot（約 6.4 分鐘）。每個 epoch 結束時，如果超過 2/3 的驗證者投票支持某個 checkpoint，該 checkpoint 就會被 justify；當連續兩個 checkpoint 都被 justify，前一個就會被 finalize。正常情況下，一筆交易從被打包到最終確認約需 2 個 epoch（~12.8 分鐘）。

本文涵蓋 LMD GHOST 的 fork choice 規則、Casper FFG 的 justify/finalize 流程，以及非正常情況（如 inactivity leak）下的行為。
