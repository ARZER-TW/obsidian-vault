...
### Block Number vs Slot vs Epoch

Ethereum 有兩套平行的計時系統，分別屬於 Execution Layer（EL）和 Consensus Layer（CL）。在閱讀文件或查詢鏈上資料時容易混淆：

| 概念 | 所屬層 | 定義 | 間隔 | 從何時開始計數 |
|------|--------|------|------|-------------|
| Block number | EL | 執行層區塊的序號，從 genesis 開始遞增 | ~12 秒（PoS 後等於 slot 間隔） | Genesis（0） |
| Slot | CL | 共識層的時間槽，每 12 秒一個 | 固定 12 秒 | Beacon Chain genesis（2020/12/1） |
| Epoch | CL | 32 個 slot 的集合 | 384 秒（6.4 分鐘） | 同上 |

> **注意**：Block number 和 slot number 不相等。The Merge 發生在 block 15537393 但 slot 4700013。此外，slot 可以為空（missed slot），此時不產生 EL 區塊，block number 不遞增但 slot number 仍遞增。
