...
在 PoS Ethereum 中，[[Beacon Chain]] 透過 [[RANDAO]] 偽隨機機制為每個 slot（12 秒）選出一個 Proposer（[[Validators|驗證者]]）。Proposer 從 [[記憶池]] 挑選交易，在 EVM 中執行，組裝出完整的區塊（包含 execution payload 和 consensus 資料），然後廣播給網路進行 [[共識與最終性|共識投票]]。
